...
contract Crowdsale {
  ...
  function Crowdsale(uint256 _numberOfTokensToCreate, uint256 _startTime, uint256 _endTime, uint256 _rate, address _wallet) {
    ...
    require(_rate > 0);
    require(_wallet != 0x0);
    require(_numberOfTokensToCreate > 0);

    token = createTokenContract(_numberOfTokensToCreate, msg.sender);
  ...
  function createTokenContract(uint256 _numberOfTokensToCreate, address tokenContractOwner) internal returns (ZackToken) {
    return new ZackToken(_numberOfTokensToCreate, tokenContractOwner );
  }
